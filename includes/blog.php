    ...
    <div class="row">
      <div class="col-md-9 blog-main">
        <div class="blog-post">
          ...
          $sql_d = "SELECT `b`.`judul`,`b`.`isi`, `b`.`tanggal`, `k`.`kategori_blog`, `u`.`nama`, `b`.`id_blog`, `b`.`sinopsis` FROM `blog` `b` INNER JOIN `kategori_blog` `k` ON `b`.`id_kategori_blog` = `k`.`id_kategori_blog` INNER JOIN `user` `u` ON `b`.`id_user`= `u`.`id_user`";
